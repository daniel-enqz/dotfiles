...
class TrailingSpaces {
     ...
     */
    getRangesToHighlight(document, selections) {
        let ranges = this.findTrailingSpaces(document);
        if (!this.settings.highlightCurrentLine) {
            let currentLines = selections.map((selection) => {
    ...
    ignoreDocument(language, scheme) {
        return (!(0, util_1.isNullOrUndefined)(language) && this.settings.languagesToIgnore[language]
            || !(0, util_1.isNullOrUndefined)(scheme) && this.settings.schemesToIgnore[scheme]);
    }
}
