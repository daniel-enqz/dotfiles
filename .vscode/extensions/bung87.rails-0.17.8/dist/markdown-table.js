...
function markdownTable(table, options) {
    ...
    while (++columnIndex < mostCellsPerRow) {
        ...
        cell = before + repeat_string_1.default('-', size) + after;
        if (settings.alignDelimiters !== false) {
    ...
    while (++rowIndex < cellMatrix.length) {
        ...
        while (++columnIndex < mostCellsPerRow) {
            ...
            if (settings.alignDelimiters !== false) {
                ...
                code = alignments[columnIndex];
                if (code === 114 /* `r` */) {
                    before = repeat_string_1.default(' ', size);
                }
                else if (code === 99 /* `c` */) {
                    if (size % 2) {
                        before = repeat_string_1.default(' ', size / 2 + 0.5);
                        after = repeat_string_1.default(' ', size / 2 - 0.5);
                    }
                    else {
                        before = repeat_string_1.default(' ', size / 2);
                        after = before;
                    }
                }
                else {
                    after = repeat_string_1.default(' ', size);
                }
            }
